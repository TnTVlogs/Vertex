...
const preview = ref<{ name: string, ownerUsername: string, memberCount: number } | null>(null)
...
const returnToApp = () => {
    // Si no està loguejat anirà a l'Auth directament
    window.location.href = '/'
}

const handleAccept = async () => {
    if (!authStore.user) {
        // Redirecció interceptada: Guarden el codi i fem saltar a la pantalla Login original
        localStorage.setItem('vertex_pending_invite', props.code)
        // Recarregar neteja el path /invite/ actual i carrega Home -> !user -> Auth
        window.location.href = '/'
        return
    }

    // Usuari Loguejat procedim a fer Join nativament
    try {
        processing.value = true
        await serverStore.joinServer(props.code)
        // Redirigeix el Root a mostrar la MainLayout un cop units
        window.location.href = '/'
    } catch (err: any) {
